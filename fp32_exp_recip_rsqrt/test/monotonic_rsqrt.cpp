...
void check_pair(
    ...
    // 非負入力は昇順なので、逆数平方根の正の出力はbit patternでも非増加となる。
    ...
    stats.maximum_increase_steps = std::max(
        ...
        static_cast<std::uint64_t>(output - previous_output));
...
void check_mantissas(VFP32ExpRecipRsqrt& dut, Stats& stats) {
    // Eの偶奇で近似関数が異なるため、指数127と128の両方を全仮数検査する。
    for (const std::uint32_t exponent : {127u, 128u}) {
        std::uint32_t previous_input = exponent << 23;
...
void check_exponent_boundaries(VFP32ExpRecipRsqrt& dut, Stats& stats) {
    const std::uint32_t fixed_boundaries[][2] = {
        {0x00000000u, 0x00000001u},
        {0x007fffffu, 0x00800000u},
        {0x7f7fffffu, 0x7f800000u},
    };
    for (const auto& pair : fixed_boundaries) {
        check_pair(
...
void report(const Stats& stats) {
    const bool pass = stats.mantissa_comparisons
                   ...
                   && stats.exponent_boundary_comparisons == 256
...
int main() {
    ...
    VFP32ExpRecipRsqrt dut{&context};
    Stats stats;
    check_mantissas(dut, stats);
