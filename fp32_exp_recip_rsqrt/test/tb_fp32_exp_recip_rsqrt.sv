...
`ifndef DUT_MODULE
`define DUT_MODULE FP32ExpRecipRsqrt
`endif

module FP32ExpRecipRsqrtTB;
    ...
    integer faithful_count [0:FUNCTION_COUNT-1];
    integer faithful_ambiguous_count [0:FUNCTION_COUNT-1];
    ...
    import "DPI-C" function int unsigned fp32_exp_recip_rsqrt_ref(
        input int unsigned input_bits,
        input int unsigned function_code
    );
    import "DPI-C" function int unsigned fp32_exp_recip_rsqrt_ref_ftz(
        input int unsigned input_bits,
        input int unsigned function_code
    );
    import "DPI-C" function longint unsigned
        fp32_exp_recip_rsqrt_faithful_bounds(
            input int unsigned input_bits,
            input int unsigned function_code
        );
    import "DPI-C" function int unsigned
        fp32_exp_recip_rsqrt_faithful_bounds_ambiguous(
            input int unsigned input_bits,
            input int unsigned function_code
        );
    import "DPI-C" function int unsigned fp32_exp_recip_rsqrt_test_input(
        input int unsigned function_index,
        input int unsigned ordinal
    );
    import "DPI-C" function int unsigned fp32_exp_recip_rsqrt_monotonic_input(
        input int unsigned function_index,
        input int unsigned ordinal,
    ...
    function automatic logic [31:0] directed_input(input integer index);
        case (index)
            ...
            14: directed_input = 32'h42b17217;
            15: directed_input = 32'h42b2d4fc;
            16: directed_input = 32'hc2cff1b4;
            17: directed_input = 32'hc2d00000;
    ...
    task automatic check_vector(
        ...
        logic [31:0] lower;
        logic [31:0] upper;
        logic faithful_eligible;
        longint unsigned ulp;
        begin
            x = input_bits;
            op = 3'(1 << index);
            #1;
            expected = REFERENCE_SUBNORMAL
                ? fp32_exp_recip_rsqrt_ref(input_bits, {29'b0, op})
                : fp32_exp_recip_rsqrt_ref_ftz(input_bits, {29'b0, op});
            ...
            if (is_nan(expected)) begin
                ...
                if (ulp > 64'(MAX_ALLOWED_ULP))
                    $fatal(1, "ULP超過: op=%h x=%h actual=%h expected=%h ulp=%0d",
                           op, x, result, expected, ulp);
            end
            faithful_eligible = index == 0
                ? input_bits[30:23] != 8'hff
                : index == 1
                    ? expected[30:23] != 0 && expected[30:23] != 8'hff
                    : !input_bits[31] && input_bits[30:23] != 0
                      && input_bits[30:23] != 8'hff;
            if (CHECK_FAITHFUL && faithful_eligible) begin
                bounds = fp32_exp_recip_rsqrt_faithful_bounds(
                    input_bits, {29'b0, op});
                lower = bounds[31:0];
                upper = bounds[63:32];
                if (fp32_exp_recip_rsqrt_faithful_bounds_ambiguous(
                        input_bits, {29'b0, op}) != 0)
                    faithful_ambiguous_count[index] =
                        faithful_ambiguous_count[index]+1;
                else if (result == lower || result == upper)
                    faithful_count[index] = faithful_count[index]+1;
                else
                    $fatal(1, "faithful違反: op=%h x=%h actual=%h lower=%h upper=%h",
                           op, x, result, lower, upper);
            end
        end
    endtask
    ...
    initial begin
        ...
        for (function_index = 0; function_index < FUNCTION_COUNT;
            ...
            faithful_count[function_index] = 0;
            faithful_ambiguous_count[function_index] = 0;
        ...
        for (issue = 0; issue < RANDOM_CYCLES; issue = issue+1)
            for (function_index = 0; function_index < FUNCTION_COUNT;
                ...
                check_vector(function_index,
                    fp32_exp_recip_rsqrt_test_input(function_index, issue));

        for (issue = 0; issue < MONOTONIC_SAMPLES; issue = issue+1)
            for (function_index = 0; function_index < FUNCTION_COUNT;
                 function_index = function_index+1) begin
                x = fp32_exp_recip_rsqrt_monotonic_input(
                    function_index, issue, MONOTONIC_SAMPLES);
                op = 3'(1 << function_index);
                #1;
                if (issue != 0) begin
                    if (function_index == 0
                        && ordered_key(result)
                           < ordered_key(previous_result[function_index]))
        ...
        for (function_index = 0; function_index < FUNCTION_COUNT;
             function_index = function_index+1) begin
            $display("FUNCTION: code=%h checked=%0d exact=%0d faithful=%0d faithful_ambiguous=%0d max_ulp=%0d monotonic_violations=%0d",
                     3'(1 << function_index), checked_count[function_index],
                     exact_count[function_index], faithful_count[function_index],
                     faithful_ambiguous_count[function_index], max_ulp[function_index],
                     monotonic_violations[function_index]);
            ...
            if (max_ulp[function_index] != 0)
                $display("WORST: code=%h x=%h actual=%h expected=%h",
                         3'(1 << function_index), worst_input[function_index],
                         worst_actual[function_index],
                         worst_expected[function_index]);
        end
        ...
        if (result != 32'h7fc00000)
            $fatal(1, "未定義opの出力不一致: op=%h actual=%h", op, result);
        op = 3'b011;
        #1;
        if (result != 32'h7fc00000)
            $fatal(1, "複数選択opの出力不一致: op=%h actual=%h", op, result);

        $display("PASS: functions=3 vectors_per_function=%0d monotonic_per_function=%0d",
