...
struct Stats {
    ...
    std::array<std::uint64_t, 64> boundary_violations{};
    std::array<Failure, 64> first_boundary_failure{};
};

std::uint32_t exp_table_index(std::uint32_t input_bits) {
    const std::uint32_t exponent = (input_bits >> 23) & 0xffu;
    if (exponent < 102u || exponent >= 134u) return 0;
...
std::uint32_t evaluate(ExpDut& dut, std::uint32_t input) {
    dut.x = input;
    dut.op = 1;
    dut.eval();
    return dut.result;
...
void check_pair(
    ...
    const std::size_t previous_table = exp_table_index(previous_input);
    ++stats.boundary_violations[previous_table];
    if (!stats.first_boundary_failure[previous_table].valid) {
        stats.first_boundary_failure[previous_table] = {
            true,
            false,
            previous_key,
            previous_input,
...
void merge_stats(Stats& target, const Stats& source) {
    ...
    for (std::size_t index = 0; index < 64; ++index) {
        target.boundary_violations[index] += source.boundary_violations[index];
        if (source.first_boundary_failure[index].valid
            && (!target.first_boundary_failure[index].valid
                || source.first_boundary_failure[index].previous_key
                    < target.first_boundary_failure[index].previous_key)) {
            target.first_boundary_failure[index] =
                source.first_boundary_failure[index];
        }
    }
}
...
void report(int threads, const Stats& stats) {
    ...
    for (std::size_t index = 0; index < 64; ++index) {
        if (stats.boundary_violations[index] == 0) continue;
        const Failure& failure = stats.first_boundary_failure[index];
        std::cout << "boundary_table=" << index
                  << " violations=" << stats.boundary_violations[index]
                  << " previous_input=0x" << std::hex << std::setw(8)
                  << std::setfill('0') << failure.previous_input
                  << " input=0x" << std::setw(8) << failure.input
                  << std::dec << std::setfill(' ') << '\n';
    }
