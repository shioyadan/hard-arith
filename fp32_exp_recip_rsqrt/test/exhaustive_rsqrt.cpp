...
std::uint32_t evaluate(VFP32ExpRecipRsqrt& dut, std::uint32_t input) {
    dut.x = input;
    dut.op = 4;
    dut.eval();
    return dut.result;
...
void merge_stats(Stats& target, const Stats& source) {
    ...
    target.contract_failures += source.contract_failures;
...
void check_representative_classes(VFP32ExpRecipRsqrt& dut, Stats& stats) {
    ...
    for (std::uint32_t sign : {0u, 0x80000000u}) {
        for (std::uint32_t exponent = 0; exponent <= 0xffu; ++exponent) {
            for (const std::uint32_t fraction : fractions) {
                ...
                const bool approximated = sign == 0 && exponent != 0
                                       && exponent != 0xffu;
                if (approximated) {
                    const std::uint64_t rne_steps = distance(output, expected);
                    stats.maximum_rne_steps = std::max(
                        stats.maximum_rne_steps, rne_steps);
                    if (rne_steps > 1) {
                        ++stats.contract_failures;
                        update_failure(stats, input, output, expected, expected);
                    }
                } else if (output != expected && !both_nan) {
                    ++stats.contract_failures;
                    update_failure(stats, input, output, expected, expected);
                }
...
int main(int argc, char** argv) {
    ...
    std::vector<std::unique_ptr<VFP32ExpRecipRsqrt>> models;
    ...
    for (int thread = 0; thread < thread_count; ++thread) {
        ...
        models.push_back(std::make_unique<VFP32ExpRecipRsqrt>(contexts.back().get()));
    }

#pragma omp parallel
    {
        const int thread = omp_get_thread_num();
        VFP32ExpRecipRsqrt& dut = *models[static_cast<std::size_t>(thread)];
...
#pragma omp for schedule(static)
        for (std::uint64_t linear = 0; linear < kExpectedSignificands; ++linear) {
            ...
            if (rne_steps > 1) {
                ++stats.contract_failures;
                update_failure(stats, input, output, bounds.rne, bounds.rne);
            }
