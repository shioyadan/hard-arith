...
#ifdef FP32_EXP_USE_QUADMATH
#include <quadmath.h>
#endif

#include "VFP32ExpRecipRsqrt.h"
...
struct Stats {
    ...
    std::uint64_t normal_rne_checks = 0;
    std::uint64_t normal_max_rne_steps = 0;
    std::uint64_t ftz_checks = 0;
    std::uint64_t ftz_mismatches = 0;
    std::uint64_t overflow_checks = 0;
    ...
    std::array<double, 64> table_minimum_ulp{};
    std::array<double, 64> table_maximum_ulp{};
    std::array<std::int64_t, 64> table_minimum_steps{};
    std::array<std::int64_t, 64> table_maximum_steps{};
    std::array<bool, 64> table_has_normal{};

    Stats() {
        table_minimum_ulp.fill(std::numeric_limits<double>::infinity());
        table_maximum_ulp.fill(-std::numeric_limits<double>::infinity());
        table_minimum_steps.fill(std::numeric_limits<std::int64_t>::max());
        table_maximum_steps.fill(std::numeric_limits<std::int64_t>::min());
    }
};

std::uint32_t exp_table_index(std::uint32_t input_bits) {
    const std::uint32_t exponent = (input_bits >> 23) & 0xffu;
    const std::uint32_t significand = 0x00800000u
                                    | (input_bits & 0x007fffffu);
    const std::uint32_t multiplier_input = ((significand >> 6) << 1) | 1u;
    const std::uint64_t product =
        static_cast<std::uint64_t>(multiplier_input) * 0x5c55u;
    const std::uint32_t shift = (5u-(exponent & 31u)) & 31u;
    const std::uint32_t window =
        static_cast<std::uint32_t>((product >> 19) & 0x7fffu) >> shift;
    const std::int32_t magnitude = static_cast<std::int32_t>(
        (window >> 1)+(window & 1u));
    const std::int32_t signed_index = (input_bits >> 31) != 0
        ? -magnitude : magnitude;
    return static_cast<std::uint32_t>(signed_index) & 63u;
}
...
void merge_stats(Stats& target, const Stats& source) {
    ...
    target.normal_rne_checks += source.normal_rne_checks;
    target.normal_max_rne_steps = std::max(
        target.normal_max_rne_steps, source.normal_max_rne_steps);
    target.ftz_checks += source.ftz_checks;
    target.ftz_mismatches += source.ftz_mismatches;
    target.overflow_checks += source.overflow_checks;
    target.overflow_mismatches += source.overflow_mismatches;
    ...
    for (std::size_t index = 0; index < 64; ++index) {
        if (!source.table_has_normal[index]) continue;
        target.table_has_normal[index] = true;
        target.table_minimum_ulp[index] = std::min(
            target.table_minimum_ulp[index], source.table_minimum_ulp[index]);
        target.table_maximum_ulp[index] = std::max(
            target.table_maximum_ulp[index], source.table_maximum_ulp[index]);
        target.table_minimum_steps[index] = std::min(
            target.table_minimum_steps[index],
            source.table_minimum_steps[index]);
        target.table_maximum_steps[index] = std::max(
            target.table_maximum_steps[index],
            source.table_maximum_steps[index]);
    }
}
...
void check_one(
...
#ifdef FP32_EXP_USE_QUADMATH
    // 最大1 ULP契約ではbinary128からbinary32へ直接丸め、
    // binary64の丸め境界誤判定を避ける。
    const __float128 exact_quad = expq(static_cast<__float128>(input));
    rounded_bits = std::bit_cast<std::uint32_t>(
        static_cast<float>(exact_quad));
#endif
    ...
    if (exact == 0.0 || (exact > 0.0 && exact < kMinimumNormal)) {
        ...
        if (output_bits != kPositiveInfinity) ++stats.overflow_mismatches;
    } else if (exact >= kMinimumNormal) {
        ++stats.normal_rne_checks;
        stats.normal_max_rne_steps = std::max(
            stats.normal_max_rne_steps, distance(output_bits, rounded_bits));
    }
    ...
    if (normal) {
        ++stats.normal_checks;
        const std::size_t table = exp_table_index(input_bits);
        stats.table_has_normal[table] = true;
        stats.table_minimum_ulp[table] = std::min(
            stats.table_minimum_ulp[table], signed_ulp);
        stats.table_maximum_ulp[table] = std::max(
            stats.table_maximum_ulp[table], signed_ulp);
        const std::int64_t signed_steps =
            static_cast<std::int64_t>(output_bits)
            - static_cast<std::int64_t>(rounded_bits);
        stats.table_minimum_steps[table] = std::min(
            stats.table_minimum_steps[table], signed_steps);
        stats.table_maximum_steps[table] = std::max(
            stats.table_maximum_steps[table], signed_steps);
        update_worst(
            stats.normal_worst,
            signed_ulp,
            input_bits,
...
Options parse_options(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--active") {
            ...
            std::exit(2);
#endif
        } else if (argument == "--by-table") {
            options.report_by_table = true;
        } else {
            std::cerr << "usage: " << argv[0]
                      << " [--active|--full] [--threads=N]"
                         " [--mpfr-near-boundary] [--by-table]\n";
            std::exit(2);
        }
    }
    return options;
...
void report(const Options& options, int threads, const Stats& stats) {
    const bool pass = stats.normal_max_rne_steps <= 1
                   && stats.ftz_mismatches == 0
                   && stats.overflow_mismatches == 0
                   && stats.special_mismatches == 0
                   && stats.negative_outputs == 0;
#ifdef FP32_EXP_USE_QUADMATH
    std::cout << "rne_oracle=expq(binary128); faithful diagnostics use "
                 FP32_EXP_ORACLE_NAME "(binary64); not a formal proof\n"
#else
    std::cout << "oracle=" FP32_EXP_ORACLE_NAME
              ...
              << "normal_rne_checks=" << stats.normal_rne_checks << '\n'
              << "normal_max_rne_steps="
              << stats.normal_max_rne_steps << '\n'
              << "ftz_checks=" << stats.ftz_checks << '\n'
              << "ftz_mismatches=" << stats.ftz_mismatches << '\n'
              << "overflow_checks=" << stats.overflow_checks << '\n'
              << "overflow_mismatches=" << stats.overflow_mismatches << '\n'
    ...
    if (options.report_by_table) {
        for (std::size_t index = 0; index < 64; ++index) {
            if (!stats.table_has_normal[index]) continue;
            std::cout << "table=" << index
                      << " minimum_ulp=" << std::setprecision(18)
                      << stats.table_minimum_ulp[index]
                      << " maximum_ulp=" << stats.table_maximum_ulp[index]
                      << " minimum_steps=" << stats.table_minimum_steps[index]
                      << " maximum_steps=" << stats.table_maximum_steps[index]
                      << '\n';
        }
    }
...
int main(int argc, char** argv) {
    ...
    std::vector<std::unique_ptr<ExpDut>> models;
    ...
    for (int thread = 0; thread < threads; ++thread) {
        ...
        models.push_back(std::make_unique<ExpDut>(contexts.back().get()));
    }
...
#pragma omp parallel num_threads(threads)
    {
        ...
        ExpDut& dut = *models[static_cast<std::size_t>(thread)];
        Stats local;
...
#pragma omp critical
    ...
    const bool pass = total.normal_max_rne_steps <= 1
                   && total.ftz_mismatches == 0
                   && total.overflow_mismatches == 0
