...
"""候補sourceのhashを照合して全入力比較topと整数係数を生成する。"""
import hashlib
import json
from generate import OUT, BANKS, VARIANTS, table

manifest=json.loads((OUT/'sources.json').read_text())
rtl=[]
header='// 自動生成。係数はQ25/Q17/Q8。bias-singleだけC0へ丸め定数を含む。\nstatic constexpr Coeff coefficients[6][3][128] = {\n'
for i,variant in enumerate(VARIANTS):
    name=variant+'/fp32_exp_recip_rsqrt.sv'
...
print('PASS: verification top and coefficients bind to six source hashes')
