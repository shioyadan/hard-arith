...
現在のRTLはfaithful rounding、すなわち厳密値を挟む直下・直上のbinary32値のどちらかを
返すことを目標にしています。directed入力と固定seedの20万乱数をsin/cosの両方で検査した
400,742評価では、faithful違反0、RNE参照からの最大差1 ULPでした。さらに小入力補正領域、
全正subnormal、主近似の全table中心を覆う区間も全数検査しています。全`2^32`入力の検査や
形式証明はまだ行っていないため、検証済み範囲を超えた保証ではありません。
...
5. sin(pi*(c+d))をcまわりの三次式で近似する
6. 固定小数点結果をbinary32へ丸める
```

この構造では、sin/cosの選択に浮動小数点加算器を使いません。位相はQ31なので、
`0.5`の加算は位相bitへの定数加算になり、その後のtableと三つの乗算器は共通です。
...
| `tools/gen_constants.py` | 主tableと小入力補正tableの生成・照合 |
...
quick testの参照値はbinary128で計算します。全数検査は対象が正の有限入力のsinに限られ、
binary32判定に対して十分広い精度を持つ`long double`を高速oracleとして使います。
いずれも形式証明や精度保証付きの計算機援用証明ではありません。
...
`make test`はNaN、無限大、signed zero、subnormal、`2^-8`の経路境界、整数、半整数、
位相が失われ始める大入力、各2の累乗、および固定seedの20万乱数をsin/cosの両方で検査します。
`make exhaustive`は小入力補正tableを使う正のnormal入力50,331,648個、正のsubnormal入力
