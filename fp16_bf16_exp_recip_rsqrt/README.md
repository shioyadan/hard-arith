...
`is_bf16` は形式の実行時切替を有効にした場合、0でFP16、1でBF16を選びます。
入力の解釈と出力形式を同時に選ぶもので、形式間の変換命令ではありません。
`x`、`op`、`is_bf16` は組合せ入力であり、各評価時に変更できます。
clock、reset、valid／ready、例外flagはありません。

## 合成時パラメータ

以下はインスタンス化時に指定し、合成時に固定する設定です。

| パラメータ | 既定値 | 設定値 | 制御する内容 |
|---|---|---|---|
| `FORMAT_MODE` | `0` | `0`／`1`／`2` | `0`: `is_bf16`でFP16／BF16を実行時切替、`1`: FP16専用、`2`: BF16専用 |
| `SUPPORT_SUBNORMAL` | `1` | `0`／`1` | `0`: 入出力FTZ、`1`: 入出力subnormal対応 |

既定値は形式の実行時切替・subnormal対応です。形式固定の`FORMAT_MODE=1/2`では
`is_bf16`を無視します。専用設定でもポート構成は同じです。
0、1、2以外の`FORMAT_MODE`はサポートしません。
`SUPPORT_SUBNORMAL`を実行時に切り替える入力はありません。

exp／recip／rsqrtは常に三機能を備え、`op`で実行時に選びます。
機能ごとの`ENABLE_*`や、入出力幅・精度を任意に変更するパラメータはありません。
各形式の数値仕様は次節のとおりです。
...
保持19 bit・guard・stickyの計21 bitを選び、符号を反映したRNE結果を20 bitの `z` にします。
...
丸めと符号反転は一つの加算へまとめます。保持整数をa、RNEの増分をb（0または1）とすると、
正側は `a+b`、負側は20-bit二の補数で `~a+(1-b)` です。
先に符号に応じてbitを反転し、増分も符号で反転して加算することで、
丸め加算の後にもう一度符号反転の加算を行う必要がなくなります。丸め規則や小数精度は変わりません。
