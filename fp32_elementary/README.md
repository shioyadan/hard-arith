...
|---|---|---|---|
| `ENABLE_EXP2` | `1` | `0`／`1` | `2^x`を無効／有効にする |
| `ENABLE_RECIP` | `1` | `0`／`1` | `1/x`を無効／有効にする |
| `ENABLE_RSQRT` | `1` | `0`／`1` | `1/sqrt(x)`を無効／有効にする |
| `ENABLE_SQRT` | `1` | `0`／`1` | `sqrt(x)`を無効／有効にする |
| `ENABLE_LOG2` | `1` | `0`／`1` | `log2(x)`を無効／有効にする |
| `ENABLE_SINCOS` | `1` | `0`／`1` | `sin(pi*x)`と`cos(pi*x)`を一括して無効／有効にする |

既定値は全機能有効です。入出力はFP32、subnormalは入出力FTZで固定され、
入出力幅・精度・subnormal対応を変更するパラメータはありません。
`op`は有効な機能の中から実行時に演算を選ぶ入力であり、合成時パラメータではありません。

### 合成時の機能選択

0にした関数の専用table、引数還元、結果選択は不要となり、合成時の定数伝播で除去できます。
sinpiとcospiは同じtableと演算経路を使うため、個別には無効化しません。
