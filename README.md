...
| ユニット | パラメータ | 制御できる内容 |
|---|---|---|
| [`FP32Exp`](fp32_exp/README.md#合成時パラメータ) | なし | FP32の自然指数関数、出力subnormal対応で固定 |
...
機能ごとの`ENABLE_*`はなく、`op`で実行時に演算を選びます。`FP32Elementary`の指数関数は
`2^x`であり、三機能版の自然指数関数`exp(x)`とは異なります。
入出力幅や精度を任意に変更するパラメータはありません。内部の`localparam`やテスト用設定は、
ここに示す公開パラメータには含めません。
